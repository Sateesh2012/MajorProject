...
function Order() {
  ...
  const userId = currentUser ? currentUser.user.uid : null; // Handle null case gracefully
    ...
    <Layout>
      ...
      {order.length > 0 ? (
        <div className="h-full pt-10">
          {order.filter(obj => obj.userid === userId).map((userOrder) => (
            <div key={userOrder.id} className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
        ...
        <h2 className='text-center tex-2xl text-white'>Not Ordered</h2>
      )}
    </Layout>
  );
}
